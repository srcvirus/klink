...
do
#	scp -i ~/.ssh/id_rsa "dnsgw-pl1.4.tar.bz2" uwaterloo_pweb@$node:~/
...
#	ssh -t -i ~/.ssh/id_rsa uwaterloo_pweb@$node "sudo cp -f ~/dnsgw /usr/local/bin/dnsgw ; sudo cp -f ~/dnsgw.conf /etc/dnsgw.conf"
	ssh -t -i ~/.ssh/id_rsa uwaterloo_pweb@$node "sudo cp -f ~/dnsgw.conf /etc/dnsgw.conf"
#	ssh -t -i ~/.ssh/id_rsa uwaterloo_pweb@$node "sudo cp -f ~/dnsgw /usr/local/bin/dnsgw"
done
