...
public:
        ...
        PeerChangeStatusMessage(string source_host, int source_port, string dest_host, int dest_port
                , OverlayID src_oid, OverlayID dst_id) : ABSMessage(MSG_PEER_CHANGE_STATUS, source_host, source_port, dest_host,
	...
	virtual ABSMessage* deserialize(char* buffer, int buffer_length)
	{
		...
		memcpy(&peer_status, buffer + offset, sizeof(int)); offset += sizeof(int);
