...
public:
    ...
    char* serialize(int* serialize_length) {
    	*serialize_length = sizeof(char) + sizeof(int) * 8 + sizeof(char) * dest_host.size() +
		...
		memcpy(buffer + offset, (char*)&hostAddrNameLength, sizeof(int)); offset += sizeof(int);
    ...
    ABSMessage* deserialize(char* buffer, int buffer_length) {
    	int offset = 0;
		int destHostLength, sourceHostLength, deviceNameLength, hostAddrNameLength;
		...
		memcpy(&hostAddrNameLength, buffer + offset, sizeof(int)); offset += sizeof(int);
		string hostAddrName = "";
		for(int i = 0; i < hostAddrNameLength; i++)
		{
			char ch;
		...
		int hPort = 0;
		memcpy(&hPort, buffer + offset, sizeof(int)); offset += sizeof(int);
