...
public:
	...
	{
		printf("[Processing thread]\tSetting next hop, Message type = %d\n",
				msg->getMessageType());
		...
		case MSG_RETRIEVE_REPLY:
			puts("[Processing thread]\treturning false");
			return false;
			break;
		}
	...
	void setUnresolvedGet(
			const LookupTable<MessageStateIndex, pair<HostAddress, string> >& unresolvedGet)
	{
	...
	void setUnresolvedPut(
			const LookupTable<MessageStateIndex, double>& unresolvedPut)
	{
