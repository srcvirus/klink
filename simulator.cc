...
//		protocol_arr[i] = new PlexusProtocol(peer_arr[i], msg_processor);
	...
	PlexusSimulationProtocol::initLocks();
				...
				else
				{
					...
					if (buffer_length > 0)
					{
						...
						if (rcvd_message != NULL)
						{
							...
							PlexusSimulationProtocol::addToIncomingQueue(rcvd_message);
...
//		message = ((PlexusProtocol*) plexus)->getIncomingQueueFront();
		...
		if (forward)
		{
			...
			PlexusSimulationProtocol::addToOutgoingQueue(message);
		}
	}
}
...
void *logging_thread(void*)
{
	...
	while (true)
	{
		...
		log->write(entry->getKeyString().c_str(),
...
void initListenSocket(const char* hosts_file)
{
	...
	for (int i = 0; i < n_hosts; i++)
	{
		...
		if (strncmp(this_host_name.c_str(), host_name,
				strlen(this_host_name.c_str())) == 0
